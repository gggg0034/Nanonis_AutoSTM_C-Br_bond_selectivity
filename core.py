...
class NanonisController(nanonis_programming_interface):
    ...
    def FolMeSpeedGet(self):
        parsedResponse = self.parse_response(
            self.send('FolMe.SpeedGet'), 'float32', 'uint32')
        return {
            'Speed': parsedResponse['0'],
            'Custom speed': parsedResponse['1']
        }
    ...
    - Error described in the Response message>Body section
    '''
    def FolMeSpeedSet(self, Speed, Custom_speed):
        if type(Speed) is str:
            Speed = self.convert(Speed)
        if type(Custom_speed) is str:
            Custom_speed = self.convert(Custom_speed)
        self.send('FolMe.SpeedSet', 'float32', Speed, 'uint32', Custom_speed)
    ...
    - Error described in the Response message>Body section
    '''
    def ZSpectrOpen(self):
        self.send('ZSpectr.Open')
