...
def construct_command(command_name, *vargs):
    ...
    for idx, arg in enumerate(vargs):
        # Except for the first command name variable in the send function, all subsequent variables alternate between variable types and variable values
        if idx % 2 == 0:                        
            datatype = arg
...
class nanonis_programming_interface:
    ...
    def __init__(self, IP = '127.0.0.1', PORT = 6501):  #6501 6502 6503 6504
    ...
    # reconnect to TCP/IP server if TCP error occurs
    # 2025-01-19 zhiwenzhu@shu.edu.cn
    def reconnect(self):
        """
        Reconnect to the specified IP and port.
        """
        try:
            self.close()
    ...
    #         true_body_size = len(body)
    #         if body_size != true_body_size:
    #             errorMessage = 'Response body size error: ' + \
    #                             returned_command + ', ' + \
    #                             str(body_size) + ', ' + \
    #                             from_binary('string', body)
                
    #             # raise nanonisException('TCP error, ready to reconnect')
    #     except:
    #         raise
    #     finally:
        ...
        while True:
            ...
            if body_size != true_body_size:
                errorMessage = 'Response body size error: ' + \
                                ...
                                str(body_size) + ', ' + \
                                from_binary('string', body)
                print('TCP error, ready to reconnect')
                self.lock.release()
                self.reconnect()
            elif body_size == true_body_size:
                self.lock.release() # Release lock
                break
    ...
    def reverse_convert(self, input_data):
        r'''
        Converts a number followed by an number * 10^{prefix exponent} into SI prefix
        ...
        Returns:
            str
        '''
        for prefix, exp in si_prefix.items():
            if abs(input_data) >= abs(exp):
                number = input_data / exp
                return f"{number:.7g}{prefix}"
        return f"{input_data:.7g}"
    ...
    #
    #################################################
    def ZCtrlOnOffSet(self, ZCtrlStatus):

        r'''
        Turn on/off the Z-controller.

        Args:
            ZCtrlStatus : Union[str, int]
                'On' or 1 to turn on the Z-controller.
                'Off' or 0 to turn off the Z-controller.

        Exceptions:
            nanonisException occurs when ZCtrlStatus is not a valid input.
        '''

        if type(ZCtrlStatus) is str:
            if ZCtrlStatus.lower() == 'on':
                ZCtrlStatus = 1
            elif ZCtrlStatus.lower() == 'off':
                ZCtrlStatus = 0
            else:
                raise nanonisException('ZCtrl On or Off?')
        elif type(ZCtrlStatus) is int:
            if ZCtrlStatus == 1:
                ZCtrlStatus = 1
            elif ZCtrlStatus == 0:
                ZCtrlStatus = 0
    ...
    def ZCtrlOnOffGet(self):
        r'''Returns the Z-controller status as a string ('On' or 'Off')'''
        parsedResponse = self.parse_response(self.send('ZCtrl.OnOffGet'), 'uint32')['0']
        if parsedResponse == 1:
            return 'On'
        elif parsedResponse == 0:
            ...
            raise nanonisException('Unknown ZCtrl State')
