...
# Zhiwen Zhu 2025/01/13    zhiwenzhu@shu.edu.cn
# Zhiwen Zhu 2025/04/10    zhiwenzhu@shu.edu.cn
import json
import os
import pickle
import random
...
class  Mustard_AI_Nanonis(NanonisController):
    def __init__(self, *args, **kwargs):
        ...
        self.molecule_registry.threshold = '0.7n'
        self.molecule_registry.threshold = self.convert(self.molecule_registry.threshold)*10**9

        self.env = Env()
        self.buffer_size = 10000
        self.buffer4log = ReplayBuffer(self.buffer_size,self.env)
        self.buffer4aug = ReplayBuffer(self.buffer_size,self.env)
        ...
        self.Scan_edge = "8n"                       # set the initial scan square edge length                           30pix  ==>>  30nm 
        self.scan_zoom_V1 = "20n"                    # the zoom of the scan, 20nm. the scale for positioning the molecule
        self.scan_zoom_V2 = "10n"                    # the zoom of the scan, 10nm. the scale for tip induce
        self.scan_zoom_V3 = "5n"                     # the zoom of the scan, 5nm. the scale for tip induce
        self.scan_zoom_in_list = [ "8n", "8n"]  # the list of the zoom in scan adge   Br[ "30n", "7n"]
        self.molecular_registration_list = []  # the list to save the molecular registration data
        self.molecular_filter_threshold = 1  # Br→4  TPM→1       # if the distance between the molecular is less than the threshold, the molecular will be filterd 
        self.zoom_in_tip_speed = '2n'               # the speed of the zoom in tip
        self.tip_manipulate_speed = '1n'            # the speed of the tip manipulate

        self.mol_scale = '1.6n'                       # the edge of the molecular, 1.8nm
        self.mol_scale = self.convert(self.mol_scale)  # the edge of the molecular, 1.8nm          1.8*1e-9
        self.Br_scale = '1.8n'                          # the edge of the Br site, 1.8nm
        self.Br_scale = self.convert(self.Br_scale)     # the edge of the Br site, 1.8nm            1.8*1e-9
        self.SAC_XYaction_edge = '1.4n'   # 4n → Br  1n → H
        self.SAC_XYaction_edge = self.convert(self.SAC_XYaction_edge)     # the edge of XY SAC, 4nm    ==>>  4*1e-9
        self.SAC_XYaction_100 = self.SAC_XYaction_edge*1e11     # 4n' ==>> 4*1e-9*1e11 ==>> 4*1e2 ==>> 400
        self.zoom_in_100 = self.convert(self.scan_zoom_in_list[-1])*1e11  # 7n' ==>> 7*1e-9*1e11 ==>> 7*1e2 ==>> 700
        self.Br_site_threshold = 0.3                # the threshold of the Br site, if the Br site is bigger than the threshold, the Br is detected
        self.H_site_threshold = 0.5                 # the threshold of the H site, if the H site is bigger than the threshold, the H is detected

        self.scan_square_Buffer_pix = 304            # scan pix
        self.plane_edge = "0.8u"                    # plane size repersent the area of the Scanable surface  2um*2um    2000pix  ==>>  2um
        ...
        self.real_scan_factor = 0.7                  # the real scan area is 80% of the scan square edge length
        ...
        self.equalization_alpha = 0.01                # the alpha of the equalization
        ...
        self.quality_model_path = './EvaluationCNN/CNN_V1.pth'  # the path of the quality model weights

        self.segment_model_path = './mol_segment/unet_model-zzw-Br_V2.pth'  # the path of the segment model weights

        self.keypoint_model_path = './keypoint/best_0417.pt'  

        self.DQN_init_moel_path = './DQN/pre-train_DQN.pth'     # the path of the pre-train DQN model

        self.site_model_path = './EvaluationCNN/CNN_Br_V3.pth'
        ...
        self.square_save_path = self.log_path + '/' + 'square_fitting'  # the path of the square fitting saving

        self.SAC_buffer_path = self.log_path + '/' +'SAC'+ '/' +'SAC_buffer'  # the path of the mol_tip_induce saving

        self.SAC_aug_buffer_path = self.log_path + '/' +'SAC'+ '/'+ 'SAC_aug_buffer'  # the path of the mol_tip_induce saving

        self.SAC_origin_action_path = self.log_path + '/' +'SAC'+ '/'+ 'SAC_origin_action'  # the path of the mol_tip_induce saving

        self.SAC_aug_origin_action_vi_path = self.log_path + '/' +'SAC'

        self.SAC_model_save_path = './SAC/train_results/'+ self.start_time  # the path of the SAC model saving

        self.SAC_model_read_path = None
        ...
        self.half_plane_size = int(self.plane_size/2)                          # the half plane size in pix  haf_plane_size = 1000
        ...
        self.R_max = self.R_init*0.9
        self.R_step = int(0.8*self.R_init)
        self.R = self.R_init
        ...
        self.mol_tip_induce_path  = self.image_data_save_path + '/' + 'mol_tip_induce'
        ...
        self.auto2SAC_queue = Queue(5)          # SAC_Br → main program
        self.SAC2auto_queue = Queue(5)          # SAC_Br → main program
        self.q_save_exp = Queue(5)                   # main program exp → SAC_Br
    ...
    # def SAC_init(self, mode = 'new'): # mode = 'new' : the SAC model is initialized to the latest model, mode = 'latest' : load the latest checkpoint
    #     if mode == 'new':
    #         self.SAC_Agent = SACAgent(self.env, self.device)
    #         # self.SAC_Agent.load_model(self.SAC_model_read_path)
    #     elif mode == 'latest':
    #         wight_list = time_trajectory_list('./SAC/train_results/',file_extension='.pth')
    #         self.SAC_Agent.load_model(wight_list[-1])
        ...
        while True:
            if not self.ScandataQueue.empty():
                ...
                if isinstance(Scan_data, dict):
                    ...
                    # to be updated
                    # if the Scan_data is a list, the Scan_data is the Scan_image
                    image_for = Scan_data['image_for']
                    image_back = Scan_data['image_back']
                    equalization_for = images_equalization(image_for, alpha=self.equalization_alpha)
                    equalization_back = images_equalization(image_back, alpha=self.equalization_alpha)
    ...
    def SAC_agent_threading(self,init_mode, auto2SAC_queue, SAC2auto_queue, q_save_exp):
        # create the SAC multiprocessing, use 
    
        env = Env(polar_space=False)
        agent = SACAgent(env)
        if init_mode == 'latest':
            wight_list = time_trajectory_list('./SAC/train_results/',file_extension='.pth')
            # get the father of wight_list
            wight_list_father = os.path.dirname(wight_list[-1])
            # agent.load_model(wight_list_father)
            agent.load_model('./SAC/train_results/2025-04-24 11-15-44', step = 29)  #   ./models/2025-03-25_21-48-29     ./models/2025-03-25_15-11-27
        elif init_mode == 'new':
            pass
        agent.train(10000,visualize=False, save_path = self.SAC_model_save_path , q_in=auto2SAC_queue,q_out=SAC2auto_queue,q_save_exp = q_save_exp)
    ...
    def move_to_next_point(self):
        if len(self.nanocoodinate_list) == 0:
            ...
            if self.tip_move_mode == 0:
                self.inter_closest = Next_inter(self.circle_list,plane_size = self.plane_size)                                       # calculate the next inter_closest
    ...
    def image_segmention(self, Scan_image):
        ...
        _,nemo_point, self.coverage = segmented_image(Scan_image, self.segmented_image_path, model_path = self.segment_model_path)    
    ...
    def image_recognition(self):
        ...
        print('The gap of the image is ' + str(linescan_max_min_check(data_linear)))
    ...
    def key_points_convert(self, key_points_result, scan_posion = (0.0 ,0.0), scan_edge = 30, w_index = False):
        out_put_key_list = []
        for idx, key_point in enumerate(key_points_result):
            # key_points_result is [[class, x, y, w, h, key_x, key_y], ......] 
            key_point_list = list(key_point)

            # key_point_list[1] = self.inter_closest[0]+(key_point[1]-0.5)*int(self.convert(self.scan_zoom_in_list[0])*10**9)
            # key_point_list[2] = self.inter_closest[1]+(key_point[2]-0.5)*int(self.convert(self.scan_zoom_in_list[0])*10**9)
            key_point_list[1] = scan_posion[0]+self.half_plane_size+(key_point[1]-0.5)*scan_edge
            key_point_list[2] = -scan_posion[1]+self.half_plane_size+(key_point[2]-0.5)*scan_edge

            key_point_list[3] = key_point[3]*scan_edge
            key_point_list[4] = key_point[4]*scan_edge
            for i in range(5, len(key_point), 2):

                # key_point_list[i] = self.inter_closest[0]+(key_point[i]-0.5)*int(self.convert(self.scan_zoom_in_list[0])*10**9) # the key point x in pixel
                # key_point_list[i+1] = self.inter_closest[1]+(key_point[i+1]-0.5)*int(self.convert(self.scan_zoom_in_list[0])*10**9) # the key point y in pixel
                key_point_list[i] = scan_posion[0]+self.half_plane_size+(key_point[i]-0.5)*scan_edge # the key point x in pixel
                key_point_list[i+1] = -scan_posion[1]+self.half_plane_size+(key_point[i+1]-0.5)*scan_edge # the key point y in pixel
                
            if w_index == False:
                key_point = tuple(key_point_list)
                out_put_key_list.append(key_point)
                # return out_put_key_list
        
            if w_index == True:
                key_point_list.append(idx)
                key_point = tuple(key_point_list)
                out_put_key_list.append(key_point)
        return out_put_key_list

    # def a function to find all moleculars in a image
    def molecular_seeker(self, image, scan_posion = (0.0 ,0.0), scan_edge = 30, one_molecular = False):
        # if the scan_posion is too small, the scan_posion is in nm, so the scan_posion should be converted to pixel
        if abs(scan_posion[0]) <= 1e-6 and abs(scan_posion[1]) <= 1e-6:
            scan_posion = list(scan_posion)
            scan_posion[0] = scan_posion[0]*10**9
            scan_posion[1] = scan_posion[1]*10**9
            scan_posion = tuple(scan_posion)
        ...
        # the format of the key_points_result is [[class, x, y, w, h, key_x, key_y], ......]
        key_points_result = key_detect(image, self.keypoint_model_path, key_point_save_dir)


        # fix a 0.02 bias in every key_points_result 

        ######################################################
        for i, key_point in enumerate(key_points_result):
            key_point = list(key_point)
            if key_point[0] == 1:  # 如果 class 是 2
                key_point[0] = 2.0  # 修改为 3.0
            elif key_point[0] == 2:  # 如果 class 是 3
                key_point[0] = 1.0  # 修改为 2.0
            key_points_result[i] = tuple(key_point)
        ######################################################

        # regsitrated the all moleculars in a regsitration list
        if len(key_points_result) == 0:                 # if no molecular is detected, return 0
            print('No molecular is detected')
            return None
        
        self.molecular_registration_list = self.key_points_convert(key_points_result,scan_posion = scan_posion, scan_edge = scan_edge)
        # filter the close moleculars
        self.molecular_registration_list = filter_close_bboxes(self.molecular_registration_list, self.molecular_filter_threshold)
        # register the moleculars
        for molecular in self.molecular_registration_list:
            key_point_list = []
            position = pix_to_nanocoordinate((molecular[5], molecular[6]), plane_size = self.plane_size, int_prosses=False)
            for i in range(5, len(molecular), 2):
                key_point = pix_to_nanocoordinate((molecular[i], molecular[i+1]), plane_size = self.plane_size, int_prosses=False)
                key_point_list.append(key_point)
        if int(molecular[0]+1) == 1:
            operated = False
        else:
            operated = True
            self.molecule_registry.register_molecule(position, key_point_list,site_states = np.array([int(molecular[0]+1)]),operated = operated)
        return key_points_result

    def single_molecular_seeker(self, image, scan_posion = (0.0 ,0.0), scan_edge = 30, one_molecular_pos = (0.0, 0.0)):
        # if the scan_posion is too small, the scan_posion is in nm, so the scan_posion should be converted to pixel
        if abs(scan_posion[0]) <= 1e-6 and abs(scan_posion[1]) <= 1e-6:
            scan_posion = list(scan_posion)
            scan_posion[0] = scan_posion[0]*10**9
        ...
        self.molecular_registration_list = self.key_points_convert(key_points_result,scan_posion = scan_posion, scan_edge = scan_edge, w_index=True)
        # filter the close moleculars
        self.molecular_registration_list = filter_close_bboxes(self.molecular_registration_list, self.molecular_filter_threshold)
        
        # find the closest molecular to the one_molecular_pos
        closest_molecular = None
        min_distance = float('inf')
        for molecular in self.molecular_registration_list:
            key_point_list = []
            position = pix_to_nanocoordinate((molecular[5], molecular[6]), plane_size=self.plane_size, int_prosses=False)
            for i in range(5, len(molecular[:-1]), 2):
                key_point = pix_to_nanocoordinate((molecular[i], molecular[i + 1]), plane_size=self.plane_size, int_prosses=False)
                key_point_list.append(key_point)
            distance = np.sqrt((position[0] - one_molecular_pos[0])**2 + (position[1] - one_molecular_pos[1])**2)
            if distance < min_distance:
                min_distance = distance
                closest_molecular = molecular
                clost_position = position
                clost_key_point_list = key_point_list
        
        if min_distance > self.molecule_registry.threshold*1e-9:
            return None
                
        # register the molecular
        if closest_molecular:
           original_index = closest_molecular[-1]
           original_key_point = key_points_result[original_index]
           self.molecule_registry.register_molecule(clost_position, clost_key_point_list)

        return [original_key_point]

    def batch_molecular_seeker(self, image, scan_posion = (0.0 ,0.0), scan_edge = 30, one_molecular_pos_list = (0.0, 0.0)):
        # if the scan_posion is too small, the scan_posion is in nm, so the scan_posion should be converted to pixel
        if abs(scan_posion[0]) <= 1e-6 and abs(scan_posion[1]) <= 1e-6:
            scan_posion = list(scan_posion)
            scan_posion[0] = scan_posion[0]*10**9
        ...
        # fix class label in every key_points_result 
        ...
        for i, key_point in enumerate(key_points_result):
            ...
            # key_points_result[i] = [key_point[j] - 0.02 if j % 2 != 0 else key_point[j] for j in range(len(key_point))]
        ######################################################
        ...
        original_key_point_list = []
        # find the closest molecular to the one_molecular_pos
        for count, one_molecular_pos in enumerate(one_molecular_pos_list):
            closest_molecular = None
            min_distance = float('inf')
            for molecular in self.molecular_registration_list:
                key_point_list = []
            ...
            if min_distance > self.molecule_registry.threshold*1e-9:
                original_key_point_list.append(None)
                closest_molecular = None
            ...
            if closest_molecular:
                original_index = closest_molecular[-1]
                original_key_point = key_points_result[original_index]
                original_key_point_list.append(original_key_point)
                if count == 0: # if the first molecular is detected, refresh the all position
                    self.molecule_registry.register_molecule(clost_position, clost_key_point_list, fresh_all_position=True)
                else:           # if the ether molecular is detected, only register don't refresh the all position
                    self.molecule_registry.register_molecule(clost_position, clost_key_point_list, fresh_all_position=False)

        return original_key_point_list
    

    # def a function to find all moleculars in a image
    def molecular_tri_seeker(self, image,n=4, scan_posion = (0.0 ,0.0), scan_edge = 30,shanp = 'triangle'):
        # if the scan_posion is too small, the scan_posion is in nm, so the scan_posion should be converted to pixel
        if abs(scan_posion[0]) <= 1e-6 and abs(scan_posion[1]) <= 1e-6:
            scan_posion = list(scan_posion)
            scan_posion[0] = scan_posion[0]*10**9
        ...
        for key_points in key_points_result:
            all_points.append((key_points[5], key_points[6]))

        all_points = np.array(all_points)
        real_points = all_points
        # print(all_points)
        # 需求：寻找边长 n=3 的金字塔三角形，共需 1+2+3=6 个点
        angle_result, center_point, length_center = compute_triangular_orientation(real_points,visualize=False,neighbor_distance=0.3)
        
        # tri_group,best_indices = find_triangle_group(real_points,n=n,rotation_angle = angle_result, dist = length_center)
        tri_key_points_result = []
        angle = angle_result

        if shanp == 'triangle':
            tri_group,best_indices = find_triangle_group(real_points,n=n,rotation_angle = angle_result, dist = length_center)
            if best_indices is None:
                print('No triangle is detected')
                return None,angle

            for i in best_indices:
                tri_key_points_result.append(key_points_result[i])
        
        if shanp == 'kagome':
            tri_group,best_indices = find_patten_group(real_points,rotation_angle = angle_result, dist = length_center,pattern= 'kagome')
            if best_indices is None:
                print('No kagome is detected')
                return None,angle
        ...
        if shanp == 'honeycomb':
            tri_group,best_indices = find_patten_group(real_points,rotation_angle = angle_result, dist = length_center,pattern= 'honeycomb')
            if best_indices is None:
                print('No honeycomb is detected')
                return None,angle

            for i in best_indices:
                tri_key_points_result.append(key_points_result[i])
        
        if shanp == 'kagome_lattice':
            tri_group,best_indices = find_patten_group(real_points,rotation_angle = angle_result, dist = length_center,pattern= 'kagome_lattice')
            if best_indices is None:
                print('No kagome lattice is detected')
                return None,angle

            for i in best_indices:
                tri_key_points_result.append(key_points_result[i])
        
        if shanp == 'honeycomb_lattice':
            tri_group,best_indices = find_patten_group(real_points,rotation_angle = angle_result, dist = length_center,pattern= 'honeycomb_lattice')
            if best_indices is None:
                print('No honeycomb lattice is detected')
                return None,angle
            ...
            tri_key_points_result = key_points_result

        elif shanp == 'costom':
            side_length = self.scan_square_Buffer_pix/(self.convert(self.scan_zoom_in_list[0])*1e9)
            # side_length = 38
            tri_key_points_result = interactive_keypoint_selector(image, key_results=key_points_result, 
                                                                  image_size=self.scan_square_Buffer_pix,
                                                                  angle=angle,
        ...
        for i, key_point in enumerate(key_points_result):
            ...
            if key_point[0] == 1:  # 如果 class 是 2
                ...
                key_point[0] = 1.0  # 修改为 2.0
            key_points_result[i] = tuple(key_point)
        # for i, key_point in enumerate(tri_key_points_result):
        #     tri_key_points_result[i] = [key_point[j] - 0.02 if j % 2 != 0 else key_point[j] for j in range(len(key_point))]
        ######################################################


        
        self.molecular_registration_list = self.key_points_convert(tri_key_points_result,scan_posion = scan_posion, scan_edge = scan_edge)
        # filter the close moleculars
        self.molecular_registration_list = filter_close_bboxes(self.molecular_registration_list, self.molecular_filter_threshold)
        # register the moleculars
        for molecular in self.molecular_registration_list:
            ...
            self.molecule_registry.register_molecule(position, key_point_list,site_states = np.array([int(molecular[0]+1)]))

        return tri_key_points_result, angle

    def molecular_tracker(self, tracker_position = (0, 0), tracker_scale = '7n', model = 'closest',move_tip = True):
        tracker_scale = self.convert(tracker_scale)
        if model == 'closest':
            molecule, molecular_index = self.molecule_registry.find_closest_molecule(tracker_position)
        if model == 'top':
            molecule, molecular_index = self.molecule_registry.find_first_unreacted_molecule()
        if molecule:
            if move_tip:
                
                # move the tip to the molecular position
                # set the folme speed to 2nm/s
                self.FolMeSpeedSet(self.zoom_in_tip_speed, 1)
                self.TipXYSet(molecule.position[0], molecule.position[1]+0.5*tracker_scale)
                self.FolMeSpeedSet(self.zoom_in_tip_speed, 0)
        return molecule, molecular_index
    ...
    while tip_in_boundary(nanonis.inter_closest, nanonis.plane_size, nanonis.real_scan_factor):
        ...
        if scan_qulity == 0:
            ...
            if molecule:
                ...
                key_points_result = nanonis.molecular_seeker(nanonis.image_for)
                if len(mask.shape) == 3:
                ...
                nanonis.molecule_registry.update_molecule(molecular_index, position = molecule.position, orientation = angle)
